...
const Sidebar = () => {
  ...
  return (
    <aside
      ref={sidebarRef}
      className={`pb-32 h-screen w-76 pr-4 h-services overflow-y-auto ${
        isSticky ? "fixed top-20 left-0" : "h-screen"
      }`}
    >
        ...
        return (
          <div key={service.slug}>
            ...
            {openService === service.slug && service.subCategories && (
              <div className="border-l-2 border-red/25 ml-6">
                {service.subCategories.map((subCategory) => {
                  const isSubActive =
                    pathname === `/hizmetlerimiz/${service.slug}/${subCategory.slug}`;

                  return (
                    <div className="" key={subCategory.slug}>
                      <Link
